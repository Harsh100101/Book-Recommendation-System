...
def upload_profile_photo():
    ...
    file_path = f"{user.username}/{filename}"

    try:
        # 1️⃣ DELETE OLD FILE IF EXISTS
        if user.profile_photo_url and "supabase.co/storage" in user.profile_photo_url:
            old_path = user.profile_photo_url.split(f"/{bucket_name}/")[-1]
            try:
        ...
        return jsonify(msg="Failed to upload to cloud storage."), 500
...
def create_admin():
    """Creates a new admin user with verified status."""
    ...
    if not re.search(r"\d", password) or not re.search(r"[^a-zA-Z0-9]", password) or len(password) < 6:
        click.echo("Error: Password must be at least 6 characters and contain one digit and one special character.")
        return
